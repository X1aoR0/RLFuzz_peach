from ZZRFuzz.ZZRFuzz_Cmplog import  *

def computeRate(init_cmp_map,cur_cmp_map,init_byte,cur_byte):
    ByteDic = []
            ...
            if cur_cmp_map.headers[i].hits == init_cmp_map.headers[i].hits:
                for j in range(min(cur_cmp_map.headers[i].hits,32)):
                    if init_cmp_map.log[i][j].v0 == init_byte and cur_cmp_map.log[i][j].v0 == cur_byte:
                        ...
                        ByteDic.append(init_cmp_map.log[i][j].v1)
                        ByteDic.append(cur_cmp_map.log[i][j].v1)
                        ...
                        ByteDic.append(init_cmp_map.log[i][j].v0)
                        ByteDic.append(cur_cmp_map.log[i][j].v0)
    return list(set(ByteDic))
