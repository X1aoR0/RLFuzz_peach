...
class FuzzMutatorPlus():
    ...
    def Mutate_ChangeASCIIInteger(self, data, loc, density):
        if len(data) > self.maxSize or len(data) == 0:
            ...
            B = loc
            while (B < len(data) and not isdigit(data[B])): B += 1 # find first byte after loc that is in '0'-'9'
            if B == len(data):
                return data
            else:
                E = B + 1
                while (E < len(data) and isdigit(data[E])): E += 1 # find first byte after B thar is not in  '0'-'9'
                l = list(data)
                digitl = l[B:E]
                val = int(bytes(digitl))
